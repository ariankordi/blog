...
This is my example demonstrating all Mii rendering features:
* Accurate Mii head models
* Wii U shader
* Body scaling
* Animation
* Headwear support
None of this is new, but it's wrapped in a clean package the way I like it. 

**screenshot: web icon example**
Now here's that same C++ code used in an example working on the web.
* It's called from JavaScript (via WASM), but this is not FFL.js.
...
* Asset size = ~950 KB.
* There is still headroom to optimize this further.

**screenshot: python and c# example**

Here is another example of that C++ code, but called from Python and C#.
...
Now for textures, which I didn't have last time, this is my example to decode those in C++.
**image**
...
* Decoders output float[] for shapes, and RGBA pixels for textures.
    ...
    * ... In particular, the Wii U deswizzler was ported from 2200 lines of C++ to just 200 lines of 2x faster code:
        * **show example image**
        * If anybody "in the know" is reading, I've only tested it to work with FFL textures. Sorry.
...
**ffl.js wink direction vs my library**
At first I thought my library messed this up, but no, I actually implemented it right before I noticed. In the original library, “left” is from the Mii’s perspective, not the viewer’s.
**real wink direction from nwf example**
...
You may ask, "what's so special about this?", and that’s a good question, because all of the expressions are just **stored in a table within FFL**.
The problem with expressions is that Nintendo overcomplicated it. Why? Because a Mii model can have multiple faces.
**image of texture dump in citra**
These are what they call “masks”, and they generate multiple mask textures for a single Mii model to allow changing expressions. You with me so far?
If you have a grid of Miis and want to use all 19 expressions with them, then you have to generate all 19 masks for each model, but more significantly, you have to load every texture needed for every one of those masks.
When we do that, we end up loading n textures for each Mii. This is the naïve approach, and you also end up loading a bunch of duplicates (e.g., eye texture is loaded twice).




I love writing code, reverse engineering, and achieving the impossible.
I think we should use existing projects when appropriate, contribute to those projects, and overall try to help others.

I do NOT like when people...
* Paste my code without any credits.
This isn't just because I want all the credits, it's also because if you don't 100% understand what it does, you will have problems.
Prime example is that **mouth color bug** that's been copied from mii2studio to many other projects by now. Nobody did their own reverse engineering or original research to come to that point, and now those same people don't know why they have this bug.
* Build an exclusive login-only website or donators-only "club"
You know exactly which one I'm talking about. It really breaks my heart to see a project and community I **continue this**
In my opinion, it's really scummy to do this with open-source code. Is it allowed? Tech **continue this**

* Rewrite code that already works for no good reason, instead of contributing to an existing project.
Every time, it makes me groan to see someone who rewrote a large library (especially now with vibe coding), not to make it architecturally better or anything, but because "now it's in my language", or "it uses this platform and not that".
With vibe coding, these people more or less don't understand what's good and bad about the original architecture and end up making something that's even worse.
It's understandable if the reason is because the old library is legacy or inflexible, but from the examples I've seen (C#, JavaScript), they make it even less flexible because you can't call them from other languages. Then what's the point?!
* Boast about their vibe-coded slop made possible by the work of others.
